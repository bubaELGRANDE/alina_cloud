        ...
        <tbody>
            <?php 
                ...
                $dataLabores = $cloud->rows("
                    SELECT
                        nombreCompleto, sucursal, cargoPersona, fechaInicioLabores,
                        DATE_FORMAT(fechaInicioLabores, '%d/%m/%Y') AS fechaInicioLaboresFormat
                    FROM view_expedientes
                    WHERE estadoPersona = ? AND estadoExpediente = ? AND MONTH(fechaInicioLabores) = ?
                    $whereSucursal
                    ORDER BY DAY(fechaInicioLabores)
                ", ['Activo', 'Activo', $_POST['selectMesAnio']]);
                $n = 0;
                foreach ($dataLabores as $labores) {
                    $n++;
                    if($labores->fechaInicioLabores == "") {
                        $antiguedad = "-";
                    } else {
                        $antiguedad = diferenciaFechasYMD(strtotime(date("Y-m-t")), strtotime($labores->fechaInicioLabores));
                    }
                    echo "
                        <tr>
                            <td>$n</td>
                            ...
                            <td>$labores->fechaInicioLaboresFormat</td>
                            <td>$antiguedad</td>
                        </tr>
                    ";
                }
    ...
    $(document).ready(function() {
        $("#btnReporteExcel").click(function(e) {
            $("#tblReporte").table2excel({
                name: `Cumpleaños por mes - Laboral - <?php echo $arrayMeses[$_POST['selectMesAnio']] . " - " . date("Y"); ?>`,
                filename: `Cumpleaños por mes - Laboral - <?php echo $arrayMeses[$_POST['selectMesAnio']] . " - " . date("Y"); ?>`
            });
        });
    });
</script>
<?php 
    function diferenciaFechasYMD($fechaPublicacion,$fechaActual) {
        ...
        $antiguiedad = substr($txtAnio, 0, -2);
