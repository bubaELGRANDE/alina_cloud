						...
						<?php 
							...
							*/
							...
							foreach($dataVendedoresComision as $dataVendedoresComision) {
								...
								$dataTotalesXVendedor = $cloud->rows("
						            SELECT
						                codTipoFactura,
						                tipoFactura,
						                correlativoFactura,
								...
								foreach($dataTotalesXVendedor as $dataTotalesXVendedor) {
							        switch ($dataTotalesXVendedor->codTipoFactura) {
							            case '1':
							                $tipoFactura = "C. FINAL";
							        ...
							        if($flgTipoCalculo == "IVA") { // CONSUMIDOR FINAL
							            // Quitar el IVA
							            $totalVendedorVenta += round(($dataTotalesXVendedor->totalVenta / $parametrizacionIVA), 2);
							        } else if($flgTipoCalculo == "NIVA") { // CRÉDITO FISCAL, EXPORTACIÓN, EXENTA
							            // Los precios ya vienen sin iva
							            $totalVendedorVenta += $dataTotalesXVendedor->totalVenta;
							        } else { 
							            // N/A = Otro tipo de documento no especificado
							            $totalVendedorVenta += 0.00;
							        }

									//$totalVendedorVenta += $dataTotalesXVendedor->totalFactura;
								}
								...
								$dataComisionVendedorVenta = $cloud->row("
									...
									FROM conta_comision_pagar_calculo
									WHERE comisionPagarPeriodoId = ? AND nombreEmpleado = ? AND flgIdentificador = 'F' AND flgComisionEditar = '0' AND flgDelete = '0'
								", [$dataPeriodo->comisionPagarPeriodoId, $dataVendedoresComision->nombreEmpleado]);
								$totalVendedorComision = $dataComisionVendedorVenta->totalVendedorComision;

								// Esto es porque la comisionPagar es INDIVIDUAL y no hay una columna que tenga la comisión completa, en los abonos no aplica porque ahí sí se calcula en una sola columna
								// Sumar las comisiones editadas
								// Validar si se han editado, ya que el valor está en una columna distinta
								$existeComisionEditadaVendedor = $cloud->count("
									SELECT 
										comisionPagarCalculoId
									FROM conta_comision_pagar_calculo
									WHERE comisionPagarPeriodoId = ? AND nombreEmpleado = ? AND flgIdentificador = 'F' AND flgComisionEditar = '1' AND flgDelete = '0'
									GROUP BY correlativoFactura, fechaFactura
								", [$dataPeriodo->comisionPagarPeriodoId, $dataVendedoresComision->nombreEmpleado]);
								if($existeComisionEditadaVendedor > 0) {
									$dataComisionVendedorVentaEditada = $cloud->rows("
										SELECT 
											comisionPagarEditar
										FROM conta_comision_pagar_calculo
										WHERE comisionPagarPeriodoId = ? AND nombreEmpleado = ? AND flgIdentificador = 'F' AND flgComisionEditar = '1' AND flgDelete = '0'
										GROUP BY correlativoFactura, fechaFactura
									", [$dataPeriodo->comisionPagarPeriodoId, $dataVendedoresComision->nombreEmpleado]);
									foreach ($dataComisionVendedorVentaEditada as $dataComisionVendedorVentaEditada) {
										$totalVendedorComision += $dataComisionVendedorVentaEditada->comisionPagarEditar;
									}
								} else {
									// No se han editado comisiones
								}
								...
								/*
								$dataComisionVendedorAbono = $cloud->row("
									SELECT 
										SUM(comisionAbonoPagar) AS totalVendedorComision 
									FROM conta_comision_pagar_calculo
									WHERE comisionPagarPeriodoId = ? AND nombreEmpleado = ? AND flgIdentificador = 'A' AND flgDelete = '0'
									GROUP BY nombreEmpleado, correlativoFactura, tipoFactura, fechaAbono, totalAbono
								", [$dataPeriodo->comisionPagarPeriodoId, $dataVendedoresComision->nombreEmpleado]);
								
								$totalVendedorComisionAbono = $dataComisionVendedorAbono->totalVendedorComision;
								*/
									...
									<tr onclick="cargarDetalleVendedor('.$n.',`'.$dataVendedoresComision->nombreEmpleado.'`,'.$dataPeriodo->comisionPagarPeriodoId.');" style="cursor: pointer;">
										...
										<td>
											<div class="row">
												...
												<div class="col-8 text-end fw-bold">$ '.number_format($totalVendedorVenta, 2, '.', ',').'</div>
											</div>
											<div class="row">
												<div class="col-4">Comisión: Ventas</div>
												<div class="col-8 text-end fw-bold">$ '.number_format($totalVendedorComision, 2, '.', ',').'</div>
											</div>
											<div class="row">
