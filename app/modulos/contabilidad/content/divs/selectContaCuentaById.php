...
$dataCC = $cloud->rows("SELECT cuentaContaId,numeroCuenta,descripcionCuenta FROM conta_cuentas_contables WHERE tipoCuenta = 'Auxiliar' AND flgDelete = ? $whereProductos ORDER by numeroCuenta ASC", [0]);

$n = 0;
foreach ($dataCC as $dataCC) {
    $n++;
    $valor = "" . $dataCC->numeroCuenta . " - " . $dataCC->descripcionCuenta;
    $json[] = ["id" => $dataCC->cuentaContaId, "text" => $valor];
}
