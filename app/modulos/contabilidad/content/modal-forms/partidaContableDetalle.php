...
$tipoPartidaId = $_POST['tipoPartidaId'] ?? 0;
$partidaContableDetalleId = $_POST['partidaContableDetalleId'] ?? 0;
...
$jsonAbrir = [
    ...
    'partidaContableDetalleId' => $partidaContableDetalleId
];
    ...
    <div class="row align-items-center mb-3">
        ...
        <div class="col-12 col-md-6 d-flex flex-column flex-md-row align-items-center justify-content-md-end gap-2">
            ...
            <button class="btn btn-warning btn-sm" type="button"
                onclick="abrirPartida(<?= htmlspecialchars(json_encode($jsonAbrir)) ?>)">
                ...
                <div class="col-md-3">
                    <div class="form-select-control">
                        <select class="form-select" style="width:100%;" <?= $readonlyOrDisabled ? 'disabled' : '' ?>
                            required id="cuentaId" name="cuentaId" tabindex="1">
...
function clearAllDet() {
    ...
    $("#abonos").val('').trigger('change');
    $("#cargos").val('').trigger('change');
    calculoDeBalance();
    ...
    let tblPartidaDet = $('#tblPartidasDetalle').DataTable({
        ...
        "initComplete": function() {
            ...
            api.columns().eq(0).each(function(colIdx) {
                let cell = $('.filtersB th').eq($(api.column(colIdx).header()).index());
                let title = $(cell).text();
        ...
        "drawCallback": function(settings) {
            const detalleId = <?php echo (int) $partidaContableDetalleId; ?>;
            if (detalleId > 0) {
                $('#tblPartidasDetalle tbody tr').each(function() {
                    const rowData = tblPartidaDet.row(this).data();
                    if (rowData && parseInt(rowData[0]) === detalleId) {
