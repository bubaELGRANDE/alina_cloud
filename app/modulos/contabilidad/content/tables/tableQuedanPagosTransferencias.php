<?php
    ...
    ", [$_POST['pagoTransferenciaId'], 'comp_quedan', 0]);
    $n = 0;
    $totalQuedan = 0;
    foreach($dataTransferenciaDetalle as $transferenciaDetalle) {
        ...
        if($transferenciaDetalle->estadoPago == "Pendiente") {
            $jsonEliminarQuedan = [
                ...
                "operation"                         => "pagos-transferencias-quedan",
                "pagoTransferenciaDetalleId"        => $transferenciaDetalle->pagoTransferenciaDetalleId
            ];

            $acciones = "
                <button type='button' class='btn btn-danger btn-sm ttip' onclick='eliminarDetalleQuedan(".htmlspecialchars(json_encode($jsonEliminarQuedan)).");'>
                    <i class='fas fa-trash-alt'></i>
                    <span class='ttiptext'>Eliminar quedan de la transferencia</span>
                </button>
            ";
        } else {
        ...
        $totalQuedan += $transferenciaDetalle->montoTransferencia;
